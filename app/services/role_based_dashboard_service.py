...
class RoleBasedDashboardService:
    ...
    @staticmethod
    def _partner_dashboard(db: Session, tenant_id: Optional[str]) -> dict:
        """
        Partner Dashboard: Partner ROI and revenue metrics.

        Shows:
        - Revenue from Partner (attributed revenue)
        - Pipeline (opportunities in partner BU)
        - Engagement Quality (candidate placement success)
        - Performance Metrics (placements, retention)
        """

        return {
            "dashboard_type": "PARTNER_ROI",
            "description": "Partner metrics - revenue, pipeline, placements, engagement",
            "featured_agents": ["Partner ROI Agent", "Engagement Tracker Agent"],
            "widgets": [
                {
                    "type": "revenue_panel",
                    "title": "Partner Revenue",
                ...
                {
                    "type": "placement_panel",
                    "title": "Successful Placements",
                    "agents": ["Thunder", "Engagement Tracker Agent"],
                },
                {
                    "type": "engagement_panel",
                    "title": "Candidate Quality",
                    "agents": ["Engagement Tracker Agent"],
                },
            ],
            "refresh_interval": 180,  # 3 min for partner
        }

    @staticmethod
    def _cfo_dashboard(db: Session, tenant_id: Optional[str]) -> dict:
        """
        CFO Agent Dashboard: AI-driven financial overview.

        Shows:
        - Revenue Forecast (AI-generated forecast)
        - Cash Flow (projections)
        - Profitability Metrics (margins, EBITDA)
        - Risk Assessment (financial health)
        - Pipeline Coverage (revenue forecast vs target)
        """

        return {
            "dashboard_type": "CFO_AGENT",
            "description": "Financial intelligence - revenue forecasts, cash flow, profitability",
            "featured_agents": ["CFO Agent", "Forecast Agent", "Margin Agent", "Risk Agent"],
            "widgets": [
                {
                ...
                {
                    "type": "cash_flow_panel",
                    "title": "Cash Flow Projection",
                    "agents": ["Cash Flow Agent"],
                },
                {
                    "type": "profitability_panel",
                ...
                },
                {
                    ...
                    "agents": ["Opportunity Tracker Agent", "Forecast Agent"],
                },
                {
                    "type": "financial_risk",
