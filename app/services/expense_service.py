...
def track_reimbursement(db: Session, user_id: Optional[str] = None) -> dict:
    """Track reimbursement status for expenses (all or by user).

    Returns summary of reimbursements in different states and detailed
    breakdown of each expense's journey through the approval workflow.
    """
    query = db.query(ExpenseRecord)

    if user_id:
        query = query.filter(ExpenseRecord.logged_by_user_id == user_id)

    expenses = query.order_by(ExpenseRecord.created_at.desc()).all()
    now = datetime.utcnow()

    total_count = len(expenses)
    pending_count = sum(1 for e in expenses if e.payment_status == "PENDING")
    approved_count = sum(1 for e in expenses if e.payment_status == "APPROVED" and e.manager_approval_status == "APPROVED")
    reimbursed_count = sum(1 for e in expenses if e.payment_status == "REIMBURSED")

    total_amount_usd_cents = sum(e.amount_usd_cents for e in expenses)
    pending_amount_usd_cents = sum(
        e.amount_usd_cents for e in expenses
        if e.payment_status in ("PENDING", "APPROVED")
    )

    reimbursements = []
    for expense in expenses:
        # Calculate days in each state
        days_pending = (now - expense.created_at).days if expense.created_at else 0

        days_awaiting_manager = 0
        days_awaiting_finance = 0

        if expense.manager_approval_status == "PENDING":
            days_awaiting_manager = days_pending
        elif expense.manager_approved_at:
            days_awaiting_manager = (expense.manager_approved_at - expense.created_at).days
            if expense.payment_status in ("PENDING", "APPROVED"):
                days_awaiting_finance = (now - expense.manager_approved_at).days

        reimbursed_at = None
        if expense.payment_status == "REIMBURSED":
            # Estimate reimbursed_at as approved_at + small buffer
            # In real system, would have explicit reimbursed_at field
            reimbursed_at = expense.created_at  # Placeholder, would be updated with dedicated field

        is_fully_processed = expense.payment_status == "REIMBURSED"

        reimbursements.append({
            "id": expense.id,
            "logged_by_user_id": expense.logged_by_user_id,
            "amount_usd_cents": expense.amount_usd_cents,
            "expense_date": expense.expense_date,
            "manager_approval_status": expense.manager_approval_status,
            "manager_approved_at": expense.manager_approved_at,
            "payment_status": expense.payment_status,
            "approved_at": expense.created_at,  # Placeholder, would be updated
            "reimbursed_at": reimbursed_at,
            "days_pending": days_pending,
            "days_awaiting_manager": days_awaiting_manager,
            "days_awaiting_finance": days_awaiting_finance,
    ...
    return {
        ...
        "total_amount_usd_cents": total_amount_usd_cents,
        "pending_amount_usd_cents": pending_amount_usd_cents,
