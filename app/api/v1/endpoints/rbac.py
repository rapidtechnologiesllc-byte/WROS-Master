...
def list_users(
    ...
    # Role filter
    if role_id:
        query = query.filter(Users.role_template_id == role_id)
    ...
    return [
        {
            ...
            "role_id": u.role_template_id,
            "role_name": u.role_template.name if u.role_template else None,
            "business_unit_id": u.bu_context.business_unit.id if u.bu_context and u.bu_context.business_unit else None,
...
# ===========================================================================
# Super User Role Template Setup (Step 1: Give Super User all permissions)
# ===========================================================================

@router.post(
    "/super-user/setup",
    response_model=dict,
    summary="Setup Super User role template with all permissions",
    dependencies=[Depends(require_permission("rbac.manage"))],
)
def setup_super_user(
    ...
    """
    Step 1 - Setup Super User role template with ALL permissions.
    Creates or updates Super User role_template with view/create/edit/delete for all resources.
    Then assigns it to superuser@blitzenx.com.
    """
    ...
    try:
        ...
        # Check if Super User role template already exists
        super_role = db.query(RoleTemplate).filter(
            RoleTemplate.name == "Super User",
            RoleTemplate.tenant_id == tenant_id
        ).first()

        if not super_role:
            # Create new Super User role template
            super_role = RoleTemplate(
                name="Super User",
                display_name="Super User",
                description="System Super User - All permissions on all resources",
                is_system=True,
                tenant_id=tenant_id,
                created_by=user.get("UserEmail", "system")
            )
            db.add(super_role)
            db.flush()
            logger.info("Created new Super User role template")
        ...
        # For each resource, create or update permission with all actions enabled
        for resource in all_resources:
            # Check if permission already exists
            existing_perm = db.query(RoleTemplatePermission).filter(
                RoleTemplatePermission.role_template_id == super_role.id,
                RoleTemplatePermission.resource_id == resource.id
            ).first()

            if not existing_perm:
                perm = RoleTemplatePermission(
                    role_template_id=super_role.id,
        ...
        # Find and assign to superuser@blitzenx.com if exists
        super_user = db.query(Users).filter(
            Users.UserEmail == "superuser@blitzenx.com",
            Users.tenant_id == tenant_id
        ).first()

        if super_user:
            super_user.role_template_id = super_role.id
            db.commit()
            logger.info(f"Assigned Super User role template to superuser@blitzenx.com (ID: {super_user.UserID})")
            return {
                "status": "success",
                "message": "Super User role template created and assigned",
                "role_template_id": super_role.id,
                "assigned_to_user": super_user.UserEmail,
                "permissions_count": len(all_resources)
            }
        else:
            logger.warning("superuser@blitzenx.com not found in database")
            return {
                "status": "partial",
                "message": "Super User role template created but superuser@blitzenx.com not found",
                "role_template_id": super_role.id,
        ...
        db.rollback()
        logger.error(f"Error setting up Super User: {exc}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to setup Super User: {str(exc)}")
