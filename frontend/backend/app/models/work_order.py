...
class WorkOrder(Base):
    ...
    po_number = Column(String(200), nullable=False, index=True)  # Client-assigned PO
    sow_reference = Column(String(500), nullable=True)  # Optional SOW/statement of work URL or ID

    # Linkages
    demand_id = Column(String(36), ForeignKey("demands.id"), nullable=False, index=True)  # DIRECT or Opportunity-sourced
    client_id = Column(String(36), ForeignKey("clients.id"), nullable=False, index=True)  # Bill-to client
    employee_id = Column(String(36), ForeignKey("employees.id"), nullable=True, index=True)  # Named resource on PO (nullable until hire)
    project_id = Column(String(36), ForeignKey("projects.id"), nullable=True, index=True)  # Auto-created when employee hired
    ...
    invoicing_contact_name = Column(String(200), nullable=True)

    # Status
    status = Column(String(50), nullable=False, default="ACTIVE")  # ACTIVE, ENDED, PAUSED, etc.
