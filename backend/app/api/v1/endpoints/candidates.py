...
class CheckReturningRequest(BaseModel):
    email: str


class CheckReturningResponse(BaseModel):
    isReturning: bool
    email: Optional[str] = None
    name: Optional[str] = None
    role: Optional[str] = None
    resumeDate: Optional[str] = None
    skillsTags: Optional[List[str]] = None
    lastApplications: Optional[List[dict]] = None


# ============ ENDPOINTS ============

@router.post("/check-returning", response_model=CheckReturningResponse)
def check_returning_candidate(request: CheckReturningRequest, db: Session = Depends(get_db)):
    """
    Check if candidate has applied before (returning candidate detection)

    Used by career site to show "Welcome back" experience for repeat applicants.
    Returns candidate's previous profile and resume data if found.
    """
    try:
        from app.models.career import CareerApplication, CareerResumeAnalysis

        # Check for career site applications first (more common for public career site)
        apps = db.query(CareerApplication).filter(
            CareerApplication.candidate_email == request.email.lower()
        ).order_by(CareerApplication.created_at.desc()).limit(1).all()

        if apps:
            # Found career application(s) - this is a returning candidate
            most_recent = apps[0]

            # Extract skills and tags from resume analysis
            skills_tags = []
            analysis = db.query(CareerResumeAnalysis).filter(
                CareerResumeAnalysis.career_application_id == most_recent.id
            ).first()

            if analysis and analysis.tags:
                skills_tags = analysis.tags[:5]  # Top 5 skills

            # Build last applications list
            last_applications = []
            for app in db.query(CareerApplication).filter(
                CareerApplication.candidate_email == request.email.lower()
            ).order_by(CareerApplication.created_at.desc()).limit(3).all():
                last_applications.append({
                    "jobId": app.career_job_id,
                    "jobTitle": f"Job ID: {app.career_job_id}",  # In production, join with CareerJob table
            ...
            return CheckReturningResponse(
                isReturning=True,
                email=most_recent.candidate_email,
                name=most_recent.candidate_name or most_recent.candidate_email.split('@')[0],
                role="Job Applicant",  # Can be enhanced with skills extraction
                resumeDate=most_recent.resume_uploaded_at.strftime('%B %d, %Y') if most_recent.resume_uploaded_at else most_recent.created_at.strftime('%B %d, %Y'),
                skillsTags=skills_tags,
                lastApplications=last_applications
            )

        # Try WROS candidates as fallback
        candidate = db.query(Candidate).filter(
            Candidate.email == request.email.lower()
        ).first()
        ...
        if candidate:
            return CheckReturningResponse(
                ...
                name=candidate.candidateName or candidate.email.split('@')[0],
                role=candidate.candidateRole or "Job Applicant",
                resumeDate=candidate.updated_at.strftime('%B %d, %Y') if candidate.updated_at else "Recently",
                skillsTags=[],
                lastApplications=[]
            )

        # No candidate found
        return CheckReturningResponse(isReturning=False)

    except Exception as e:
        # Graceful fallback - log error but don't crash
        print(f"Error checking returning candidate: {str(e)}")
        return CheckReturningResponse(isReturning=False)
