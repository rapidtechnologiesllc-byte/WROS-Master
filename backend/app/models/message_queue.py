...
class MessageQueue(Base):
    ...
    type = Column(String(100), nullable=False, index=True)  # e.g., 'candidate_added', 'interview_scheduled'
    queue_type = Column(String(50), nullable=True, index=True)  # Channel-based: THUNDER_QUEUE, EMAIL_QUEUE, etc.
    status = Column(String(50), nullable=False, index=True)  # PENDING, SLM_PROCESSING, CHANNEL_QUEUED, COMPLETED, FAILED
    ...
    email_status = Column(String(50), nullable=True)  # PENDING, SENDING, SENT, DELIVERED, OPENED, CLICKED, REPLIED, BOUNCED, SPAM, DELETED
    ...
    email_provider = Column(String(50), nullable=True)  # gmail, outlook, yahoo, apple, smtp
...
class MessageChannel(Base):
    ...
    queue_type = Column(String(50), nullable=False, index=True)  # Which channel to route to
...
class EmailTracking(Base):
    ...
    provider = Column(String(50), nullable=False)  # gmail, outlook, yahoo, apple, smtp
    message_id_external = Column(String(255), nullable=True)  # External message ID from provider
    thread_id = Column(String(255), nullable=True)  # Gmail thread ID
    status = Column(String(50), nullable=False, default="PENDING")  # PENDING, SENT, DELIVERED, OPENED, CLICKED, REPLIED, BOUNCED, SPAM, DELETED
...
class EmailTrackingEvent(Base):
    ...
    id = Column(String(36), primary_key=True, default=_new_uuid)
    tracking_id = Column(String(36), nullable=False, index=True)
    event_type = Column(String(50), nullable=False)  # sent, delivered, opened, clicked, replied, bounced, spam, deleted
...
class QueueProcessingState(Base):
    ...
    id = Column(String(36), primary_key=True, default=_new_uuid)
    queue_type = Column(String(50), nullable=False, unique=True, index=True)
    last_processed_message_id = Column(String(36), nullable=True)
