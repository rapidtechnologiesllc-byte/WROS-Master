...
class AgentConfig(Base):
    ...
    # Agent Identity
    name = Column(String(100), nullable=False, unique=True)  # Unique key: "thunder", "recruitment_screener"
    display_name = Column(String(200), nullable=False)  # "AI Recruiter", "Screening Bot"
    description = Column(Text, nullable=True)  # What does this agent do?

    # Queue Configuration
    queue_name = Column(String(100), nullable=False)  # Input queue for this agent
    next_queue_name = Column(String(100), nullable=True)  # Output queue to next agent
