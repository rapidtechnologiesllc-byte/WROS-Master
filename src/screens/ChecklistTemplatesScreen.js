...
export default function ChecklistTemplatesScreen() {
  ...
  return (
    <div className="grid gap-4">
      <Card title="Checklist templates" icon={<ListChecks className="h-4 w-4" />}>
        <p className="mb-4 text-sm text-gray-600">
          Create reusable templates for pre-onboarding. Assign them to candidates from the
        ...
        {createOpen ? (
          <div className="mb-6 rounded-2xl border bg-gray-50 p-4">
            <div className="text-xs font-semibold text-gray-500">Create template</div>
              ...
              <div key={t.id} className="rounded-2xl border bg-white">
                <div className="flex flex-wrap items-center justify-between gap-2 p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button variant="secondary" onClick={() => handleExpand(t.id)}>
                    ...
                    {detailLoading ? (
                      ...
                      <>
                              ...
                              >
                                ...
                                <div className="flex flex-wrap items-end gap-2">
                                  <Select
                                    ...
                                    value={it.item_type}
                                    onChange={(v) =>
                                      handleQuickUpdateItem(it.id, { item_type: v })
                                    }
                                    options={["todo", "queue"]}
                                  />
                                  <Button
                        ...
                        <div className="mt-4 rounded-xl border bg-white p-3">
                          <div className="text-xs font-semibold text-gray-700">Add item</div>
                          <div className="mt-2 grid gap-3 md:grid-cols-2">
                            ...
                            <Select
                              ...
                              onChange={(v) => setItemForm((f) => ({ ...f, item_type: v }))}
                              options={["todo", "queue"]}
                            />
                            <Input
                              label="Order index"
                              ...
                              onChange={(v) =>
                                setItemForm((f) => ({ ...f, order_index: Number(v) || 0 }))
                              }
                            />
                            <Input
                              label="Due days offset (optional)"
                              value={itemForm.due_days_offset === "" ? "" : String(itemForm.due_days_offset)}
                              onChange={(v) =>
                                setItemForm((f) => ({
                                  ...f,
                      ...
                      <div className="text-sm text-gray-500">Could not load template.</div>
                    )}
                  </div>
                ) : null}
              </div>
